/**** 
    ...
    *   Change Date: 2018.12.10    better 禁止浏览器默认行为
    ****/
/*
 * 移动端 完美 理想视口
...
/**** better 禁止浏览器默认行为 2018.12.10 ****/
function preventEvent(event) {
    if (event.cancelable) {
        event.preventDefault();
    } else {
        console.log('kjf_Error: This event couldn\'t be canceled.');
        console.dir(event);
    }
}

// 检查回调函数
function checkRun(callBack, eventStr, argv){
    var reg = new RegExp(eventStr);
    if(callBack){    // 封装，执行 callBack 对象中的所有 dragenter 方法
        for(var funcName in callBack){
            if( reg.test(funcName) &&
                typeof callBack[funcName] === "function"){
                callBack[funcName](argv);
            };
        };
    };
};
